 ...
 * ----------  --------  ----------  -----------------------------------------*
 * 2017/07/01  V1.00.00  Edson       收單-分期付款 Tips t 處理程式                 *
...
/*收單-分期付款 Tips t 處理程式*/
public class BilO208 extends BilOBase {
 

// **************************************************************************
 public BilO208(AuthTxnGate pGate, AuthGlobalParm pGlobalParm, TableAccess pTableAccess, String sPTxIndicator) throws Exception {
	 progname = "收單-分期付款 Tips t 處理程式  106/07/01 V1.00.00";
  ...
  select_bil_install_log();
  if (hRespCd.equals("00")) {
	  gTableAccess.updateBilInstallLog2(this);
 ...
 /***********************************************************************/
 private void select_bil_install_log() throws Exception {
	 ...
	 hContInstallTotTerm = Integer.parseInt(hProdCd);
	 hContContractNo = hOrderNo;
	 hContFirstRemdAmt = hAmt;
 ...
 private void genReturnData() throws Exception {
  ...
  gInstallTxData.setReturnMesgType("0230");
  ...
  */
  ...
  gInstallTxData.setValue3(tempX12);
  return;
 }
